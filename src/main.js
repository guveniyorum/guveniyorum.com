...
import('./platform-store.js').then(({ platformStore }) => {
  ...
  function userKey() { return user ? `user:${user.id || user.email || user.displayName || 'active'}` : 'guest'; }
  ...
  platformStore.supabase?.auth.onAuthStateChange((_event, session) => { if (session?.user?.email) complete(session.user.email, session.user.app_metadata?.provider || 'Supabase'); });
});
